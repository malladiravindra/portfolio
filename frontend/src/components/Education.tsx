...
export default function Education({ entries }: { entries: TimelineEntry[] }) {
  ...
  return (
    <section id="education" className="container-page scroll-mt-16 pb-16 sm:pb-24">
      ...
      <div className="flex max-w-2xl flex-col gap-4">
        {entries.map((entry, i) => {
          const [degree, org] = entry.org.split(' · ')
          return (
            <motion.div
              key={entry.hash}
              ...
              className="flex gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-card transition-all duration-300
